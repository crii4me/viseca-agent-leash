...
The JSON Schema lives at `schema/mandate.schema.json` (regenerate with
`python scripts/export_schema.py`). **Function 2 should validate against that
file, not import these Pydantic models** — it keeps the two functions
independently testable and lets a non-Python consumer check the same contract.
